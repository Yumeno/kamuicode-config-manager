...
1. リポジトリに `DRIVE_FILE_ID` シークレットを設定（設定済みの場合はスキップ）
2. Actionsタブから手動実行でワークフローをテスト
3. 動作確認後、定期実行を有効化

### 修正履歴

#### 2025-11-30: 変更検出ロジックの修正

**問題**: GitHub Actionsが完走してもYAMLファイルがコミットされない

**原因**: `git diff --quiet` は **既存ファイル（tracked）の変更** のみ検出し、**新規ファイル（untracked）** は検出できなかった

**修正内容**:
- `Check for changes` ステップを削除
- `Commit and push` ステップで `git add` 後に `git diff --cached` でステージングされた変更をチェックするように変更
- 新規ファイルも既存ファイルの変更も正しく検出・コミットできるようになった
