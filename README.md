...
**Note**: 最新情報を取得するには「カタログクリア」後に再度読み込んでください。

> ⚠️ **自動更新の停止について（2026-05-25）**
> モデル定義（`kamuicode_model_memo.yaml`）およびツールカタログ（`mcp_tool_catalog.yaml`）の **自動更新は停止しました**。
> 以降、これらのデータは更新されません。表示内容は2026-05-25時点の情報であり、最新のkamuicodeサーバー構成とは異なる場合があります。
